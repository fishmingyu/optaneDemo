...
{
    args::ArgumentParser parser("Optane SPMM test.", "This goes after the options.");
    args::HelpFlag help(parser, "help", "Display this help menu", {'h', "help"});
    args::ValueFlag<std::string> dax_path(parser, "dax_path", "Mapped dax file", {'d', "dax_path"});
    args::ValueFlag<int> memory_type(parser, "memory_type", "Memory type to use, 0 for DRAM, 1 for OPTANE", {'m', "memory_type"});
    args::ValueFlag<std::string> sparse_file(parser, "sparse_file", "Sparse matrix file path", {'s', "sparse_file"});
    ...
    readMtx<float>(sparse_file.Get().c_str(), row_COO, col_COO, values_COO, nrow, ncol, nnz);
    COO_to_CSR(row_CSR, row_COO, nnz, nrow);
    Array<int> csrptr(nrow, myOptane);
